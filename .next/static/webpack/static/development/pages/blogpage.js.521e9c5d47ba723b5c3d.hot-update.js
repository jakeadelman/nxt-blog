...
function (_React$Component) {
  ...
  function BlogContent(props) {
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "fetchImg", function (entry) {
      _this.client.getAssets(entry);

      console.log("fetching asset ", entry);
    });
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "setPosts", function (response) {
      ...
      _this.setState({
        posts: response,
        markdown: marked(res),
        heroImage: response.fields.heroImage.sys.id
      });

      console.log(_this.state.heroImage);
    });
  ...
  _createClass(BlogContent, [{
    ...
    value: function componentDidMount() {
      this.fetchPosts(this.props.id).then(this.setPosts).then(this.fetchImg(this.state.heroImage).then(this.setImg));
    }
  }, {
    key: "render",
...
//# sourceMappingURL=blogpage.js.521e9c5d47ba723b5c3d.hot-update.js.map